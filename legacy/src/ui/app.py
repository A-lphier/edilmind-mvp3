...
# Config Streamlit
st.set_page_config(
    page_title="EdilMind - Matching Intelligente Bandi",
    page_icon="???",
    layout="wide",
...
# HEADER
# ============================================================================
st.title("??? EdilMind - Sistema Matching Intelligente Bandi")
            ...
            if not text:
                ...
                with col_a:
                    regione = st.selectbox(
                        "Regione",
                        [""] + sorted(["Abruzzo", "Basilicata", "Calabria", "Campania", "Emilia-Romagna", "Lazio", "Liguria", "Lombardia", "Marche", "Piemonte", "Puglia", "Sardegna", "Sicilia", "Toscana", "Trentino-Alto Adige", "Umbria", "Veneto"])
                    )
                with col_b:
    ...
    if not supabase:
        ...
        if not bandi:
            ...
            for bando in bandi_filtrati[:20]:
                with st.expander(f"?? {bando['cig']} - {bando.get('titolo', 'Senza titolo')[:80]}"):
                    col_a, col_b = st.columns(2)
...
# ============================================================================
    ...
    with st.form("profilo_impresa"):
        ...
        with col_imp2:
            importo_max = st.number_input("Importo massimo capacit� (�)", value=1000000, step=100000)
...
# ============================================================================
    ...
    if 'profilo_impresa' not in st.session_state:
        st.warning("?? Configura prima il profilo impresa nella sezione 'Profilo Impresa'")
    elif not supabase:
        st.warning("?? Configura Supabase per vedere i matching")
    else:
